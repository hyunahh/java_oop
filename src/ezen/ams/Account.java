...
//		this.restMoney = 0L;
	...
	// 출금기능
	public long withdraw(long money) throws NotSufficientBalnceException {

		if (money <= 0) {
			throw new NotSufficientBalnceException("출금금액은 음수일 수 없습니다.", 110);
		}

		if (money > restMoney) {
			// 예외를 개발자가 발생시킴
			throw new NotSufficientBalnceException("잔액이 부족합니다.", 100);
		}
		restMoney -= money;
		return restMoney;
	}
